 ...
 */
export function gatherChildren(root: TSMT$INode): Array<TSMT$INode>
{
  if (root == null) {
    return [];
  }

  let n: TSMT$INode = root.children;
 ...
 */
export const TSMT_CLUSTER_DEFAULT_PARAMS:Object = {
  clusters: 2
};
...
export class TSMT$SimpleGraph
{
  ...
  public static toTree(mst: Array<TSMT$IEdge>): TSMT$INode | null
  {
    ...
    let list: Array<TSMT$IEdge> = mst.slice();  // shallow copy is okay
   ...
   */
  public static getNode(id: string, root: TSMT$INode): TSMT$INode
  {
    ...
    let list: TSMT$INode = root.children;
    let node: TSMT$INode;
  ...
  protected __preorderTraversal(node: TSMT$INode)
  {
    ...
    let list: TSMT$INode = node.children;
  ...
  public cluster(data: TSMT$ClusterData,
                 ...
                 params: Object=TSMT_CLUSTER_DEFAULT_PARAMS,
  ...
  protected __clusterByNumber(clusters: number): Array<TSMT$INode>
  {
    ...
    const theTrees: Array<TSMT$INode> = [ TSMT$SimpleGraph.toTree(this._mst) ];
  ...
  protected __findNode(node: TSMT$INode): TSMT$INode | null
  {
    ...
    const n: TSMT$INode = this.__find(node);
  ...
  {
    const rootX: TSMT$INode = this.__findNode(x);
    const rootY: TSMT$INode = this.__findNode(y);
